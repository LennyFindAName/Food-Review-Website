...
public class UserSettingDAO extends DBContext {
    ...
    public void updateRememberMe(boolean status, int settingId){
        String sql = "UPDATE `user_setting` SET `is_remember_me` = ? WHERE (`user_setting_id` = ?);";
         try {
            PreparedStatement ps = connection.prepareStatement(sql);
    ...
    public UserSetting findSettingIdByUserId(String userId){
